...
| D2 | Use `express@^5.2.1` (current `latest`) | `express@4.22.2` (`latest-4`) | Default `npm install express` resolution; Node v22 satisfies `engines >= 18`; greenfield with no v4 legacy | Express 5 changed routing (path-to-regexp v8); mitigated by the pathless catch-all (D3) |
...
| 8 | *(no source — new feature R2)* | `app.get('/good-evening', ...)` → `res.send('Good evening')` |
